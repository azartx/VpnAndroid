...
class FeatureApiManager<F : AbstractFeatureApi> internal constructor(
    ...
    private val scopeCallback = object : ScopeCallback {
        override fun onScopeClose(scope: Scope) {
            println("TAGGGG---onScopeClose------------------- ${scope.toString()}")
            if (childScopes.remove(scope.id)) {
                if (!feature.isLibrary && childScopes.isEmpty()) {
                    println("TAGGGG---onScopeClose-------------------2222222 ${featureScope.toString()}")
                    featureScope.close()
                }
            }
        }
    ...
    fun link(childScope: Scope) {
        println("TAGGGG---link-------------------------- ${childScope.toString()}")
    ...
    private fun createScope(): Scope {
        println("TAGGGG---createScope-------------------------- ${toString()}")
        loadScopeIfNeeded()
    ...
    private fun loadScopeIfNeeded() {
        println("TAGGGG---loadScopeIfNeeded-------------------------- ${toString()}")
